export const FETCH_PROJECTS_REQUEST="FETCH_PROJECTS_REQUEST";
export const FETCH_PROJECTS_SUCCESS="FETCH_PROJECTS_SUCCESS";
export const FETCH_PROJECTS_FAILURE="FETCH_PROJECTS_FAILURE";

export const FETCH_PROJECTS_BY_ID_REQUEST="FETCH_PROJECTS_BY_ID_REQUEST";
export const FETCH_PROJECTS_BY_ID_SUCCESS="FETCH_PROJECTS_BY_ID_SUCCESS";
export const FETCH_PROJECTS_BY_ID_FAILURE="FETCH_PROJECTS_BY_ID_FAILURE";

export const CREATE_PROJECTS_REQUEST="CREATE_PROJECTS_REQUEST";
export const CREATE_PROJECTS_SUCCESS="CREATE_PROJECTS_SUCCESS";
export const CREATE_PROJECTS_FAILURE="CREATE_PROJECTS_FAILURE";

export const SEARCH_PROJECTS_REQUEST="SEARCH_PROJECTS_REQUEST";
export const SEARCH_PROJECTS_SUCCESS="SEARCH_PROJECTS_SUCCESS";
export const SEARCH_PROJECTS_FAILURE="FETCH_PROJECTS_FAILURE";

export const DELETE_PROJECTS_REQUEST="DELETE_PROJECTS_REQUEST";
export const DELETE_PROJECTS_SUCCESS="DELETE_PROJECTS_SUCCESS";
export const DELETE_PROJECTS_FAILURE="DELETE_PROJECTS_FAILURE";

export const INVITE_TO_PROJECTS_REQUEST="INVITE_PROJECTS_REQUEST";
export const INVITE_TO_PROJECTS_SUCCESS="INVITE_PROJECTS_SUCCESS";
export const INVITE_TO_PROJECTS_FAILURE="DELETE_PROJECTS_FAILURE";

export const ACCEPT_INVITATION_PROJECTS_REQUEST="ACCEPT_INVITATION_PROJECTS_REQUEST";
export const ACCEPT_INVITATION_PROJECTS_SUCCESS="ACCEPT_INVITATION_PROJECTS_SUCCESS";
export const ACCEPT_INVITATION_PROJECTS_FAILURE="ACCEPT_INVITATION_PROJECTS_FAILURE";